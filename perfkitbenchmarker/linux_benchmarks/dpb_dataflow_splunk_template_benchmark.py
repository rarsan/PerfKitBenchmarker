...
BENCHMARK_NAME = 'dpb_dataflow_splunk_template_benchmark'

BENCHMARK_CONFIG = """
dpb_dataflow_splunk_template_benchmark:
...
def Run(benchmark_spec):
  ...
  for template_param in template_params:
    param_kv = template_param.split('=')
    metadata[param_kv[0]] = param_kv[1]
  metadata.update({'template_uri': template_gcs_location})
  ...
  avg_throughput = dpb_service_instance.GetAvgOutputThroughput(
      output_ptransform, start_time, end_time)
  results.append(sample.Sample('avg_throughput', avg_throughput, '1/s',
                               metadata))
  ...
  results.append(sample.Sample('outbound-successful-events',
      dpb_service_instance.GetMetricValue('outbound-successful-events'),
      'number', metadata))

  results.append(sample.Sample('outbound-failed-events',
      dpb_service_instance.GetMetricValue('outbound-failed-events', 0),
      'number', metadata))

  results.append(sample.Sample('avg_write_to_splunk_batch',
      dpb_service_instance.GetMetricValue('write_to_splunk_batch_MEAN'),
      'number', metadata))

  results.append(sample.Sample('avg_write_to_splunk_latency',
      dpb_service_instance.GetMetricValue('successful_write_to_splunk_latency_ms_MEAN'),
      'ms', metadata))
